...
from runtime.agent import run_livekit_session
...
async def lifespan(app: FastAPI):
    logger.info("AgentOS starting — mode=LIVEKIT (Mode 1)")
    yield
...
async def join_session(req: JoinRequest):
    ...
    task = asyncio.create_task(
        run_livekit_session(session_id, room_name, agent_token),
        name=f"session-{session_id}",
    )
    _sessions[session_id] = task
    task.add_done_callback(lambda t: _sessions.pop(session_id, None))

    logger.info(f"session.start mode=livekit id={session_id}")
    return {
