...
from shared.providers.llm import stream_llm
from shared.providers.tts import RawCartesiaTTS
from shared.providers.stt import DeepgramSTT
from shared.tools.registry import TOOL_DEFINITIONS
...
class RealtimeSession:
    def __init__(self, session_id: str, room_name: str, agent_token: str):
        ...
        self.telemetry = TelemetryEmitter(session_id, 3, "hybrid")
    ...
    async def on_stt_result(self, transcript: str, is_final: bool):
        ...
        if self.state.agent_speaking and len(transcript.strip()) >= 3:
            ...
            asyncio.create_task(self.telemetry.emit("interruption.detected", {"text": transcript}))
        ...
        if is_final:
            ...
            asyncio.create_task(self.telemetry.emit("user.transcript.final", {"text": transcript}))
            if self.metrics:
    ...
    async def _run_agent_turn(self, context_id: str):
        ...
        try:
            asyncio.create_task(self.telemetry.emit("llm.request.start", {"context_id": context_id}))
            async for item in stream_llm(messages, tools=TOOL_DEFINITIONS):
                if item["type"] == "content":
                    ...
                    if first_chunk and self.metrics:
                        ...
                        asyncio.create_task(self.telemetry.emit("llm.first_token", {"token": token}))
                        first_chunk = False
                    ...
                    for sentence in sentences:
                        ...
                        asyncio.create_task(self.telemetry.emit("tts.request.start", {"text": sentence}))
                    ...
                    for tc in item["tool_calls"]:
                        ...
                        asyncio.create_task(self.telemetry.emit("tool.call.start", {"tool": name, "args": args}))
                        ...
                        asyncio.create_task(self.telemetry.emit("tts.request.start", {"text": filler}))
                        ...
                        asyncio.create_task(self.telemetry.emit("tool.call.complete", {"tool": name, "args": args, "result": result}))
                        await self.room.local_participant.publish_data(
            ...
            if leftover:
                ...
                asyncio.create_task(self.telemetry.emit("tts.request.start", {"text": leftover}))
            ...
            if full_response.strip():
                ...
                asyncio.create_task(self.telemetry.emit("llm.complete", {"text": full_response.strip()}))
            ...
            asyncio.create_task(self.telemetry.emit("pipeline.error", {"error": str(e), "stage": "llm"}))
        ...
        try:
            async for pcm_chunk in self.tts.receive_audio_chunks(context_id):
                if first_audio and self.metrics:
                    ...
                    asyncio.create_task(self.telemetry.emit("tts.first_audio", {"context_id": context_id}))
                    first_audio = False
    ...
    async def run(self):
        ...
        try:
            asyncio.create_task(self.telemetry.emit("session.lifecycle", {"status": "started"}))
            ...
            while True:
                ...
                if participant_joined and len(self.room.remote_participants) == 0:
                    logger.info("[RUNTIME] Room empty, draining session")
                    break
                elif not participant_joined and wait_time > 30:
                    logger.info("[RUNTIME] No participant joined within 30s timeout, draining session")
                    break
            ...
            asyncio.create_task(self.telemetry.emit("session.lifecycle", {"status": "closed"}))
